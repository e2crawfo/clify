...
clify_doc = """ Turn a function into a script that accepts arguments from the command line.
...
class CommandLineFunction(object):
    ...
    def __init__(
        ...
        self.defaults, self.parser = self._build_arg_parser(**build_parser_kwargs)

    def _build_arg_parser(
            self, wrapped, verbose, cl_args, allow_abbrev,
            collect_kwargs, parser, message):
        ...
        if parser is None:
            message = message or "Automatically generated argument parser for wrappedtion {}.".format(wrapped.__name__)
            try:
                parser = argparse.ArgumentParser(message, allow_abbrev=allow_abbrev)
            except TypeError:
                if not allow_abbrev:
                    warnings.warn("clify argument ``allow_abbrev`` set to False, but abbrevation wrappedtionality "
